...
def coarsen(A, levels, self_connections=False):
    ...
    # [print('perms len',len(x)) for x in perms]
    ...
    return graphs, perms[0] if levels > 0 else None,perms,parents
...
def compute_perm(parents):
    ...
    for parent in parents[::-1]:
        ...
        for i in indices[-1]:
            ...
            # print('child nodes: {}'.format(indices_node))
            # print('node: {}'.format(i))
            # time.sleep(2)
            ...
            # print('indices layer',indices_layer)
...
compute_perm([np.array([4,1,1,2,2,3,0,0,3]),np.array([2,1,0,1,0])])